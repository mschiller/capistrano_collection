...
#Capistrano::Configuration.instance(true).load do
  set :ferret_host, 'localhost'
  set :ferret_env , 'production'
  ...
  namespace :ferret do
    ...
    task :stop, :roles => :app do
      ...
      run "cd #{current_path}; #{ruby} script/ferret_server --root . -e #{rails_env} stop || true; echo return_code: $?"
    end
    ...
    task :start, :roles => :app do
      ruby = fetch(:ruby, 'ruby')
      rails_env = fetch(:rails_env, 'production')
      run "cd #{current_path}; #{ruby} script/ferret_server --root . -e #{rails_env} start"
    end
    ...
    namespace :index do
      ...
      task :rebuild, :roles => :app do
        rails_env = fetch(:rails_env, 'production')
        run "cd #{current_path}; RAILS_ENV=#{rails_env} #{rake_path} ferret:index:rebuild:all"
      end
    ...
    task :symlink, :roles => :app do
      run "mkdir -p  #{shared_path}/index && rm -rf #{current_path}/tmp/index && ln -nfs #{shared_path}/index #{current_path}/tmp/index"
    end

    desc "install ferret gem with sudo"
    ...
    task :cleanup, :roles => :app do
      ...
      indexes.each do |index|
        ...
        if count >= releases.length
          ...
          puts "[Ferret cleanup] no old indexes to clean up"
        else
